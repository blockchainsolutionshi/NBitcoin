...
namespace NBitcoin.Altcoins.Elements
{
	public class ElementsCoinSelector<TNetwork> : DefaultCoinSelector
	{
		public override IEnumerable<ICoin> Select(IEnumerable<ICoin> coins, IMoney target)
		{
			void getAssetAmounts(IMoney money, Dictionary<uint256, long> dictionary)
			{
				...
				switch (money)
				{
					case MoneyBag _moneyBag:
						foreach (var mbi in _moneyBag)
						{
	...
	public abstract class ElementsTransactionBuilder : TransactionBuilder
	{
		...
		//		throw new ArgumentOutOfRangeException(nameof(asset), "asset can't be negative");
		...
		//internal class ElementsSendBuilder : SendBuilder
		//{
		...
		//	public override void Build(TransactionBuildingContext ctx)
		//	{
		//		_ = base.Build(ctx);
		...
		protected override void AfterBuild(Transaction transaction)
		{
			...
			{
				var totalInput = this.FindSpentCoins(transaction)
					.Select(c => c.TxOut)
					...
					.OfType<Money>()
					.Sum();
				var totalOutputs = transaction.Outputs.OfType<ElementsTxOut>()
					.Where(o => o.IsPeggedAsset == true)
					.Select(o => o.Value)
					.Sum();
				var fee = totalInput - totalOutputs;
				if (fee > Money.Zero)
